...
export class PodiumBridge {
	...
	#sendMessage(message) {
		...
		// @ts-ignore
		if (globalThis.window.webkit && globalThis.window.webkit.messageHandlers) {
			// @ts-ignore
			globalThis.window.webkit.messageHandlers.nativebridgeiOS.postMessage(
				message,
			);
			// @ts-ignore
		} else if (globalThis.window.NativeBridgeAndroid) {
			// @ts-ignore
			globalThis.window.NativeBridgeAndroid.send(JSON.stringify(message));
		} else {
	...
	destroy() {
		...
		globalThis.window.removeEventListener(EVENT_NAME, this.#receiveMessage);
	}
}
