...
export class PodiumBridge {
    ...
    on(method, eventHandler) {
        ...
        (this.#handlers[method] = this.#handlers[method] || []).push(
            eventHandler,
        );
    }
    ...
    #receiveMessage(event) {
        const message =
            /** @type {RpcRequest<unknown> | RpcResponse<unknown>} */ (
                event.detail
            );
        if (typeof message !== 'object')
            throw new Error('message must be an object');
        ...
        if (typeof message.jsonrpc !== 'string' || message.jsonrpc !== '2.0')
            throw new Error(
                `JSON-RPC version ${message.jsonrpc} is unsupported`,
            );
...
if (typeof window !== 'undefined') {
    // @ts-expect-error We add it in fixup.sh
